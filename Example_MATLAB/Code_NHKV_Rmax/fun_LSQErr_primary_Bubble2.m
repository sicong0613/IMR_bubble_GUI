function [LSQErr] = fun_LSQErr_primary_Bubble2(var2fit, ...
    t_exp, tspan_nondim, t_nondim_exp, ...
    Req_m, R_nondim_exp, Rmax_um, ...
    t_win_s, R_exp_win_m)
...
[~, idx_sim_max] = max(R_sim_nondim);
t_sim_max = t_sim(idx_sim_max);
t_sim = t_sim - t_sim_max;


LSQErr = fun_LSQErr_interpolation(t_sim, R_sim, t_win_s, R_exp_win_m);


% %%
% 
% % ---------- Sample simulation at experimental times (interpolation) ----------
% %Make sure simulation time is increasing for interp1
% % (ode45 returns increasing t, but keep it safe)
% [t_sim_nondim_sorted, order] = sort(t_sim_nondim(:));
% R_sim_nondim_sorted = R_sim_nondim(order);
% 
% % Interpolate simulation R onto experimental t points (within first cycle)
% R_sim_at_exp = interp1(t_sim_nondim_sorted, R_sim_nondim_sorted, ...
%                        t_win_s, 'linear', 'extrap');
% 
% % ---------- LSQ error (first cycle only) ----------
% [~, idx_exp_max] = max(R_exp_win_m);
% idx_exp_end = length(R_exp_win_m);
% N_exp = idx_exp_end - idx_exp_max + 1;
% 
% [~, idx_sim_max] = max(R_sim_at_exp);
% 
% R_sim_after_max = R_sim_at_exp(idx_sim_max:end);
% [~, N_sim] = min(R_sim_after_max);
% idx_sim_end = idx_sim_max + N_sim - 1;
% 
% if N_exp < N_sim
%     R_exp_res = R_exp_win_m(idx_exp_max:end);
%     R_sim_res = R_sim_at_exp(idx_sim_max : idx_sim_max + N_exp - 1);
% else
%     R_sim_res = R_sim_at_exp(idx_sim_max : idx_sim_end);
%     R_exp_res = R_exp_win_m(idx_exp_max : idx_exp_max + N_sim - 1);
% end
% 
%     res = R_exp_res(:) - R_sim_res(:);
% 
% 
% 
% 
% % R_exp_res = R_exp_win_nondim(idx_exp_max:end);
% % 
% % res = R_exp_win_nondim(:) - R_sim_at_exp(:);
% 
% % Option A: plain SSE
% LSQErr = sum(res.^2);
% 
% % Option B (recommended): MSE to remove dependence on sample count
% % LSQErr = mean(res.^2);
