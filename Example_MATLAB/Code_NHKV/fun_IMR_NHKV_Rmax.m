...
% close all; clear; clc;
% Generate random G, u
% Set the number of random numbers you want to generate
Num = 100000;  
...
mu_log_max_value = 0;
% 
% Define the range of lambdamax = Rmax/R0 
lambdamax_min_value = 1.1;
lambdamax_max_value = 13;
% 
% Generate n random numbers between 0 and 1
random_numbers_G = rand(Num, 1);
random_numbers_lambdamax = rand(Num, 1);
random_numbers_mu = rand(Num, 1);
% 
% uniform deformation in the range of G,mu, Rmax
G_log_random_numbers = random_numbers_G * (G_log_max_value - G_log_min_value) + G_log_min_value;
lambdamax_random_numbers = random_numbers_lambdamax * (lambdamax_max_value- lambdamax_min_value) + lambdamax_min_value;
mu_log_random_numbers = random_numbers_mu * (mu_log_max_value - mu_log_min_value) + mu_log_min_value;


% Go over {G,mu,lambdamax} selections to get R/R0 vs t/t0 curve
for j = 1 : Num
    % Parameters to be specified
    disp(['Test #', num2str(j)]);
    % Test 10% PAAM + 0% Alginite
    %G = 1e3%13.82e3
    %mu = 1e-3%0.0954
    %lambdamax = 5.9989
    G = 10^G_log_random_numbers(j); disp(['G = ', num2str(G), ' Pa']);
    mu = 10^mu_log_random_numbers(j); disp(['mu = ', num2str(mu), ' Pa s']);
    lambdamax = lambdamax_random_numbers(j); 
    disp(['lambda_max = ', num2str(lambdamax)]);
   
    %%%%% Undeformed bubble radius %%%%%
    R0 = 35e-6; % Undeformed (equilibrium) bubble radius (m) %%%%% TODO
    ...
    if strcmp(cav_type,'LIC') == 1
        Rmax = lambdamax*R0; % Maximum (initial) bubble radius for LIC (m) %%%%% TODO
    ...
    c_long = 1700%1700%1550;  % Longitudinal wave speed (m/s)
    alpha = 0.0 % alpha = 0.0;   % Strain-stiffening parameter (1) (alpha=0: neo-Hookean) %%%%% TODO
    ...
    %%%%%% Interpolate %%%%%%%
    % change units to 1e-4
    scale = 1e4;
    t_newunit = t * scale;  % unit: 0.1 ms
    R_newunit = R * scale;  % unit: 0.1 mm
    
    [~,RminInd] = min(R_newunit);
    % [~,tmaxInd] = max(t_newunit);

    t_interp_range = 0.8
    
    %if RminInd==length(R_newunit) 
        % If there is no bubble rebound, skip this parameter choice and do not save R-t curve
        % disp('no bubble rebound');
        % continue;
        % 

    %else
        % If there is a bubble rebound, modify R-t curve to assign 0 for R after first collapse
        % R_newunit(RminInd+1:end) = 0;
        
    %end
    %% Judgement range of t

    %if RminInd~=length(R_newunit) & t_newunit(end) <= t_interp_range
    if R_newunit(1)>=R_newunit(length(R_newunit)) & t_newunit(end) <= t_interp_range
        % redo above part
        % Original tspan = 6*tc

        tspan = 6*tc;%tspan = 3*tc; 
        ...
        input_fileID = fopen('E:\dataset_202401001\Prediction\pAAM_old_round2\additional_dataset\formal_data_v4\input.txt', 'a');
        
        % input_fileID2 = fopen('E:\new_dataset_04162024\new_dataset_1000_new_range\input2.txt', 'a');
        output_fileID = fopen('E:\dataset_202401001\Prediction\pAAM_old_round2\additional_dataset\formal_data_v4\output.txt', 'a');
        log_output_fileID = fopen('E:\dataset_202401001\Prediction\pAAM_old_round2\additional_dataset\formal_data_v4\log_output2.txt', 'a');
        Rmax_fileID = fopen('E:\dataset_202401001\Prediction\pAAM_old_round2\additional_dataset\formal_data_v4\Rmax2.txt', 'a');
        
        % Time increments at equal intervals of 1e-7 (10 million fps)
        t_interp_newunit = 0:0.008:t_interp_range;
        % t_interp = 0.68/0.005;

        % Interpolate using pchip method
        R_interp_newunit = interp1(t_newunit, R_newunit, t_interp_newunit, 'pchip');
        R_interp = R_interp_newunit/scale;

    
        figure, plot(t,R); hold on, plot(t_interp_newunit, R_interp_newunit, 's' );
        
        % save R_interpolated
        for m = 1:length(t_interp_newunit)-1
            fprintf(input_fileID, '%f ', R_interp_newunit(m));
            % fprintf(input_fileID2, '%f ', R_interpolated(m)*100);
        end
        ...
        fprintf(input_fileID, '\n'); % R-t curves
        %fprintf(input_fileID2, '\n'); % R-t curves
        fprintf(output_fileID, '%d ', G); % G
        fprintf(output_fileID, '%.3f\n', mu); % mu

        fprintf(log_output_fileID, '%d ', G_log_random_numbers(j)); % log10G
        fprintf(log_output_fileID, '%.3f\n', mu_log_random_numbers(j)); % log10mu 
        fprintf(Rmax_fileID, '%d\n', lambdamax); % Rmax
    
        fclose(input_fileID);
        % fclose(input_fileID2);
        fclose(output_fileID);
        fclose(log_output_fileID);
        fclose(Rmax_fileID);
        
        % draw R-t curve
        %{
        figure(j);
        %plot(t_newunit, R_newunit , 'k-','linewidth',1); % bubble R vs. t curve
        scatter(t_interp_newunit, R_interp_newunit, 's')
        %hold on;
        %scatter(t_interp_newunit, R_interp_newunit, 10, 'red', 'filled');
        xlabel('$t$ (1e-4s)','Interpreter','latex','Fontsize',16);
        ylabel('$R$ (1e-4m)','Interpreter','latex','Fontsize',16);
        grid on;
        %hold off;

        figure(j+1);
        %plot(t_newunit, R_newunit , 'k-','linewidth',1); % bubble R vs. t curve
        scatter(t_nondim, R_nondim, 's')
        %hold on;
        %scatter(t_interp_newunit, R_interp_newunit, 10, 'red', 'filled');
        xlabel('$t_{nondim}$ ','Interpreter','latex','Fontsize',16);
        ylabel('$R_{nondim}$ ','Interpreter','latex','Fontsize',16);
        grid on;
        %hold off;
        %}
