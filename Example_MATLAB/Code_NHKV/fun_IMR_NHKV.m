function [t_sim,R_sim,U_sim,P_sim, t_sim_nondim,R_sim_nondim] = fun_IMR_NHKV(var2fit, t_nondim_exp, R_nondim_exp, Req_exp, tspan2fit_exp)
% This function is to calculate the least square error between the
% experimetnally measured bubble radius R1 vs time curve and the numerically
% simulated bubble R1 vs time curve
%
%   INPUTS: var2fit: Variables to fit
%           t_nondim_exp: Nondimensionalized time points from exp data
%           R_nondim_exp: Nondimensionalized bubble radii from exp data
%           R_eq:         Equlibirum bubble radius, unit: um
%
%   OUPUT: LSQErr: Least square errors between the simulated and
%                  experimentally measured bubble radius vs time curves
%
%   Author: Sicong Wang, Jin Yang
%   Contact: sicongw@utexas.edu
%   Date: 08/28/2025
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
...
%%
    ...
    if strcmp(cav_type,'LIC') == 1
        Rmax = R0; % Maximum (initial) bubble radius for LIC (m) %%%%% TODO
    ...
    params = [NT C_star We Ca alpha Re Rv Ra kappa fom chi A_star B_star Pv_star Req_nondim PA_star omega_star delta_star n];
    ...
    if strcmp(cav_type,'LIC') == 1
        ...
        Se0 = (3*alpha-1)*(5 - 4*Req_nondim - Req_nondim^4)/(2*Ca) + ...
    ...
    Se_nondim = X_nondim(:,4); % Stress integral history
   ...
   [Rmax_sim, Rmax_sim_ind] = max(R_sim);
    ...
    % Rsim_fit_pp1 = interp1(t_sim_nondim,R_sim_nondim,'pchip','pp');
    % R_nondim_sim_fit = ppval(Rsim_fit_pp1, t_nondim_exp);

    % Calculate the LSQ error
    % index_err = find(t_nondim_exp < tspan2fit_exp, 1, 'last');
    % LSQErr = sum((R_nondim_exp(1:index_err) - R_sim_nondim(1:index_err)).^2); 

    % disp(['LSQErr: ', num2str(LSQErr)]);
...
function dxdt = bubble(t,x,cav_type,params)
    ...
    if strcmp(cav_type,'LIC') == 1
        ...
        Sedot = 2*U/R*(3*alpha-1)*(1/Rst + 1/Rst^4)/Ca - ...
    ...
    dxdt = [rdot; udot; pdot; Sedot; Theta_prime; k_prime];
